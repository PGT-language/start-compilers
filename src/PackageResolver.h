...
class PackageResolver {
  ...
  static void add_candidate(std::vector<std::string> &candidates,
  ...
  static void add_import_candidates(std::vector<std::string> &candidates,
    ...
    if (!package_file.empty()) {
      ...
      add_candidate(candidates,
                    join_path(base, (package_path / package_file).string()));
    }
  }
  ...
  static std::string stdlib_path_for(const std::string &import_path) {
    ...
    if (starts_with(import_path, "std/") || starts_with(import_path, "std\\")) {
      return (std::filesystem::path("src/stdlib") / import_path.substr(4))
          .string();
    }
    return import_path;
  }
  ...
  static std::vector<ModuleRequirement>
  read_module_requirements(const std::string &root) {
  ...
  static std::string import_suffix_for_module(const std::string &import_path,
                                              const std::string &module_path) {
  ...
  collect_package_files(const std::string &directory) {
    std::vector<std::string> files;
...
public:
  PackageResolver(const std::string &entry_file,
                  const std::string &executable_path)
      : project_root(directory_of(entry_file)),
        compiler_root(directory_of(executable_path)) {
  ...
  ResolvedImport resolve_import_path(const std::string &base_dir,
    ...
    if (standard) {
      ...
      add_import_candidates(file_candidates, compiler_root,
                            stdlib_path_for(import_path));
      ...
      add_directory_candidate(directory_candidates, compiler_root,
                              stdlib_path_for(import_path));
      add_directory_candidate(directory_candidates, ".",
                              stdlib_path_for(import_path));
      ...
      for (const auto &requirement : read_module_requirements(project_root)) {
        ...
        std::filesystem::path module_base =
            std::filesystem::path(project_root) / ".pgt" / "pkg" /
            module_cache_name(requirement.path);
        std::string suffix =
            import_suffix_for_module(import_path, requirement.path);
        std::filesystem::path cached_import =
            suffix.empty() ? module_base : module_base / suffix;
        add_import_candidates(file_candidates, "", cached_import.string());
        add_directory_candidate(directory_candidates, "",
                                cached_import.string());
      }
    }
    ...
    std::string fallback = file_candidates.empty()
                               ? with_pgt_extension(import_path)
                               : file_candidates.front();
  ...
  void validate_main_package_root(const std::string &entry_file) const {
    ...
    for (const auto &entry : std::filesystem::directory_iterator(root_dir)) {
      ...
      if (package_name.empty()) {
        throw SemanticError(
            "File in main package root has no package declaration: '" +
                file_path + "'",
            SourceLocation(1, 0, file_path));
      }
      if (package_name != "main") {
        throw SemanticError(
            "Main package root cannot contain package '" + package_name +
  ...
  void validate_package_directory(const std::string &file_path,
    ...
    if (actual_name != package_name) {
      throw SemanticError(
          "Package directory mismatch: file declares package '" + package_name +
