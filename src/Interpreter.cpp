...
std::string Interpreter::log_level_from_builtin(const std::string &name) const
{
    ...
    if (name == "log_critical" || name == "log_critecal" || name == "log_fatal")
        return "CRITICAL";
    if (name == "log::critical" || name == "log::critecal" || name == "log::fatal")
        return "CRITICAL";
    return "INFO";
}
...
void Interpreter::run(const std::vector<std::shared_ptr<AstNode>> &program)
{
    ...
    for (const auto &node : program)
    {
        ...
        else if (auto var = std::dynamic_pointer_cast<VarDecl>(node))
        {
            ...
            if (DEBUG)
                std::cout << "[DEBUG] Global var " << var->name << " = " << val.to_string() << std::endl;
        }
    }
...
Value Interpreter::execute_function(const std::string &name, const std::vector<Value> &call_args)
{
    ...
    if (DEBUG)
        std::cout << "[DEBUG] Function " << name << " has " << func->param_names.size() << " params, got " << call_args.size() << " args" << std::endl;
            ...
            bin->op == T_LESS_EQUAL || bin->op == T_EQUAL_EQUAL || bin->op == T_NOT_EQUAL)
        {
            ...
            else
            {
                    ...
                    l.type == ValueType::NONE || r.type == ValueType::NONE)
                {
                    ...
                    if (DEBUG)
                        std::cout << "[DEBUG] Comparing: '" << l_str << "' " << (bin->op == T_GREATER ? ">" : bin->op == T_LESS      ? "<"
                    ...
                    if (DEBUG)
                        std::cout << "[DEBUG] Comparison result: " << (result ? "true" : "false") << std::endl;
                }
                else
                {
                    // Сравнение чисел
