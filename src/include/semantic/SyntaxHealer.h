...
private:
  ...
  static bool can_end_expression(TokenType type) {
    return type == T_IDENTIFIER || type == T_INPUT ||
           type == T_STRING_LITERAL || type == T_NUMBER || type == T_TRUE ||
           type == T_FALSE || type == T_RPAREN;
  }
  ...
  static bool can_start_expression(TokenType type) {
    return type == T_IDENTIFIER || type == T_INPUT ||
           type == T_STRING_LITERAL || type == T_NUMBER || type == T_TRUE ||
           type == T_FALSE || type == T_LPAREN || type == T_READ;
  }
  ...
  static bool is_namespaced_root(const Token &token) {
    ...
    return token.value == "web" || token.value == "net" ||
           token.value == "log" || token.value == "json" ||
           token.value == "auth" || token.value == "jwt" ||
           token.value == "sql" || token.value == "orm" ||
           token.value == "request";
  }
  ...
  static bool expects_paren_after(const std::vector<Token> &tokens,
                                  size_t index) {
    const Token &token = tokens[index];
    if (token.type == T_FUNCTION || token.type == T_IF ||
        token.type == T_CALL || token.type == T_COUT || is_print(token.type)) {
      return true;
    }
  ...
  static bool
  should_insert_equal_after_type(const std::vector<HealingToken> &out,
                                 const std::vector<Token> &tokens,
                                 size_t index) {
    if (out.size() < 3 || !is_type(out.back().token.type))
      return false;
    if (out[out.size() - 2].token.type != T_PLUS)
      return false;
    if (out[out.size() - 3].token.type != T_IDENTIFIER)
      return false;
    if (index + 1 >= tokens.size())
      return false;

    TokenType next_type = tokens[index + 1].type;
    if (next_type == T_EQUAL || next_type == T_COMMA || next_type == T_RPAREN ||
        next_type == T_RBRACE || next_type == T_EOF) {
      return false;
    }
  ...
  static bool should_close_paren_before(const std::vector<HealingToken> &tokens,
    ...
    if (stack.empty() || stack.back().type != T_LPAREN)
      return false;
    const Token &current = tokens[index].token;
    if (current.type == T_RPAREN || current.type == T_COMMA)
      return false;
    if (!can_end_expression(previous.type))
      return false;
    if (current.type == T_LBRACE || current.type == T_RBRACE)
      return true;
  ...
  static bool should_close_brace_before(const std::vector<HealingToken> &tokens,
                                        ...
                                        const std::vector<Token> &stack,
                                        const Token &previous) {
    if (stack.empty() || stack.back().type != T_LBRACE)
      return false;
    const Token &current = tokens[index].token;
    if (current.type == T_ELSE) {
      return previous.type != T_RBRACE && can_end_statement(previous.type);
    }
    if (current.type == T_RETURN && index + 1 < tokens.size() &&
        tokens[index + 1].token.type == T_NUMBER &&
        tokens[index + 1].token.value == "0" && previous.type == T_NUMBER &&
        previous.value == "1" && current.line > previous.line) {
      return true;
    }
  ...
  static bool
  should_insert_comma_before(const std::vector<HealingToken> &tokens,
                             size_t index, const std::vector<Token> &stack,
                             const Token &previous) {
    if (stack.empty() || stack.back().type != T_LPAREN)
      return false;
    const Token &current = tokens[index].token;
    if (current.type == T_LPAREN)
      return false;
    if (!can_end_expression(previous.type) ||
        !can_start_expression(current.type)) {
      return false;
    }
  ...
  static TextEdit make_insert(int line, int column, const std::string &text,
                              const std::string &message, size_t order) {
    TextEdit edit;
  ...
  static TextEdit make_erase(const Token &token, const std::string &message,
                             size_t order) {
    TextEdit edit;
  ...
  static TextEdit make_replace_range(int line, int column, size_t length,
                                     const std::string &replacement,
                                     const std::string &message, size_t order) {
    TextEdit edit;
    edit.kind = TextEdit::Kind::Replace;
  ...
  static TextEdit insert_after_token(const Token &token,
                                     ...
                                     const std::string &message, size_t order) {
    return make_insert(
        token.line, token.column + static_cast<int>(source_token_length(token)),
        text, message, order);
  }
  ...
  static TextEdit
  replace_gap_after_token(const std::string &source, const Token &previous,
                          const Token &current, const std::string &text,
                          const std::string &message, size_t order) {
    TextEdit edit;
    edit.kind = TextEdit::Kind::Replace;
    edit.line = previous.line;
    edit.column =
        previous.column + static_cast<int>(source_token_length(previous));
    edit.text = text;
    edit.order = order;
    edit.message = message;
  ...
  static std::string close_message(TokenType type) {
    if (type == T_RPAREN)
      return "missing ')' was inserted";
    if (type == T_RBRACE)
      return "missing '}' was inserted";
    return "missing token was inserted";
  }

  static TextEdit close_edit(const std::string &source, TokenType close_type,
                             const Token &current, bool at_eof,
                             const Token &previous, size_t order) {
    ...
    if (close_type == T_RBRACE) {
      if (current.type == T_RETURN && previous.type == T_NUMBER &&
          previous.value == "1" && current.line > previous.line) {
        return replace_gap_after_token(source, previous, current,
                                       "\n" + text + "\n\n", message, order);
      }
  ...
  static std::vector<HealingToken> normalize(const std::vector<Token> &tokens,
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
      ...
      if (expects_paren_after(tokens, i) &&
        ...
        Token inserted = synthetic(
            T_LPAREN, token.line,
            token.column + static_cast<int>(source_token_length(token)));
        out.push_back({inserted, true});
        if (edits) {
          edits->push_back(insert_after_token(
              token, "(", "missing '(' was inserted", edits->size()));
        }
      }
  ...
  static void close_top(std::vector<Token> &out, std::vector<Token> &stack,
                        const std::string &source, std::vector<TextEdit> *edits,
                        const Token &current, bool at_eof,
                        const Token &previous) {
    ...
    if (edits) {
      edits->push_back(close_edit(source, close_type, current, at_eof, previous,
                                  edits->size()));
    }
  }
      ...
      if (should_insert_comma_before(tokens, i, stack, previous)) {
        ...
        if (edits) {
          if (token.line == previous.line && token.column > 0) {
            edits->push_back(insert_before_token(
                token, ",", "missing ',' was inserted", edits->size()));
          } else {
            edits->push_back(insert_at_line_end(source, previous, ",",
      ...
      if (is_close(token.type)) {
        ...
        if (skip_current || stack.empty()) {
          if (edits && !tokens[i].synthetic) {
            edits->push_back(make_erase(
                token, "extra closing token was removed", edits->size()));
          }
          continue;
        }
  ...
  static std::string lower_ascii(const std::string &value) {
    ...
    for (char raw_ch : value) {
      lowered +=
          static_cast<char>(std::tolower(static_cast<unsigned char>(raw_ch)));
    }
    return lowered;
  }

  static size_t edit_distance(const std::string &left,
  ...
  static size_t typo_score(const std::string &word,
                           const std::string &candidate, bool strong_context) {
    std::string lowered_word = lower_ascii(word);
    std::string lowered_candidate = lower_ascii(candidate);
    if (lowered_word == lowered_candidate)
      return 0;

    size_t distance = edit_distance(lowered_word, lowered_candidate);
    if (distance <=
        typo_threshold(lowered_word, lowered_candidate, strong_context)) {
      return distance;
    }
    return impossible_score();
  }

  static size_t impossible_score() { return static_cast<size_t>(-1) / 4; }

  static bool is_statement_start(const std::vector<Token> &tokens,
                                 size_t index) {
    if (index == 0)
      return true;
    const Token &previous = tokens[index - 1];
    const Token &current = tokens[index];
    return previous.line < current.line || previous.type == T_LBRACE ||
           previous.type == T_RBRACE;
  }

  static bool is_identifier_token(const Token &token) {
    return token.type == T_IDENTIFIER || token.type == T_READ ||
           token.type == T_WRITE || token.type == T_FILE ||
           token.type == T_OBJECT;
  }
  ...
  static void add_unique_word(std::vector<std::string> &values,
                              const std::string &word) {
    if (word.empty() || contains_word(values, word))
      return;
  ...
  static std::vector<std::string> plain_builtin_words() {
    return {"protocol",     "json_parse",   "json_stringify",
            "read_file",    "open_log",     "request_method",
            "request_path", "request_body", "request_json",
            "log",          "log_output",   "set_log_output",
            "log_console",  "log_file",     "log_trace",
            "log_debug",    "log_info",     "log_notice",
            "log_warn",     "log_warning",  "log_error",
  ...
  callable_words(const std::vector<Token> &tokens) {
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
      ...
      if (i + 2 < tokens.size() && tokens[i + 1].type == T_LPAREN &&
  ...
  static void add_replace_if_needed(std::vector<TextEdit> &edits,
                                    const Token &token,
                                    const std::string &replacement) {
    if (replacement.empty() || token.value == replacement)
      return;
    edits.push_back(make_replace(token, replacement,
  ...
  static bool tokens_are_adjacent(const Token &left, const Token &right) {
    return left.line == right.line &&
           right.column ==
               left.column + static_cast<int>(source_token_length(left));
  }

  static bool can_follow_number_suffix_removal(TokenType type) {
    return type == T_EOF || type == T_RBRACE || type == T_RPAREN ||
           type == T_COMMA || type == T_PLUS || type == T_MINUS ||
           type == T_STAR || type == T_SLASH || type == T_GREATER ||
           type == T_LESS || type == T_EQUAL_EQUAL || type == T_NOT_EQUAL ||
  ...
  static void add_numeric_suffix_edits(std::vector<TextEdit> &edits,
    ...
    const Token &token = tokens[index];
    const Token &previous = tokens[index - 1];
    if (token.type != T_IDENTIFIER || previous.type != T_NUMBER)
      return;
    if (!tokens_are_adjacent(previous, token))
      return;
    ...
    edits.push_back(make_erase(
        token, "invalid numeric suffix '" + token.value + "' was removed",
        edits.size()));
  }

  static bool has_package_main(const std::vector<Token> &tokens) {
    for (size_t i = 0; i + 1 < tokens.size(); ++i) {
      if (tokens[i].type == T_PACKAGE && tokens[i + 1].type == T_IDENTIFIER &&
  ...
  static int brace_depth_before(const std::vector<Token> &tokens,
                                size_t index) {
    int depth = 0;
  ...
  static void add_main_exit_return_edits(std::vector<TextEdit> &edits,
                                         const std::vector<Token> &tokens,
                                         size_t index, bool package_main) {
    if (!package_main || index + 1 >= tokens.size())
      return;
    const Token &token = tokens[index];
    const Token &value = tokens[index + 1];
    if (token.type != T_RETURN || value.type != T_NUMBER)
      return;
    ...
    edits.push_back(make_replace(
        value, "0", "main exit code '" + value.value + "' was corrected to '0'",
        edits.size()));
  }
  ...
  static bool add_malformed_network_call_edits(std::vector<TextEdit> &edits,
    ...
    if (index + 6 < tokens.size() && is_network_transport(first_part) &&
      ...
      if (end_column <= start_column)
        return false;
      edits.push_back(make_replace_range(
          outer_open.line, start_column,
          static_cast<size_t>(end_column - start_column), "::",
          "malformed network transport call was normalized", edits.size()));
      return true;
    }

    if (is_network_method(first_part) && is_network_method(tokens[index + 4]) &&
        first_part.value == tokens[index + 4].value &&
        tokens[index + 5].type == T_LPAREN &&
        outer_open.line == tokens[index + 5].line) {
      int start_column = outer_open.column + 1;
      int end_column = tokens[index + 5].column +
                       static_cast<int>(source_token_length(tokens[index + 5]));
      if (end_column <= start_column)
        return false;
      edits.push_back(make_replace_range(
          outer_open.line, start_column,
          static_cast<size_t>(end_column - start_column), "",
          "duplicated network method wrapper was removed", edits.size()));
      return true;
    }
  ...
  static void add_namespace_typo_edits(std::vector<TextEdit> &edits,
    ...
    if ((index > 0 && tokens[index - 1].type == T_COLON_COLON) ||
        (index + 3 < tokens.size() &&
         tokens[index + 3].type == T_COLON_COLON)) {
      return;
    }
  ...
  static bool should_correct_statement_word(const std::vector<Token> &tokens,
    ...
    if (candidate == "function" || candidate == "class" || candidate == "if" ||
        candidate == "while" || candidate == "call" || candidate == "cout" ||
        candidate == "print" || candidate == "printg" ||
        candidate == "println") {
      return statement_start &&
             (next_type == T_LPAREN || next_type == T_IDENTIFIER);
    }
    return statement_start && token.column > 0;
  }

  static std::vector<std::string>
  strong_statement_words(const std::vector<Token> &tokens, size_t index) {
  ...
  static bool has_keyword_prefix_damage(const std::string &word,
    ...
    std::string lowered_candidate = lower_ascii(candidate);
    size_t prefix = 0;
    while (prefix < lowered_word.size() && prefix < lowered_candidate.size() &&
  ...
  static bool has_repeated_extra_suffix(const std::string &word,
    ...
    std::string lowered_word = lower_ascii(word);
    std::string lowered_candidate = lower_ascii(candidate);
    if (lowered_word.size() <= lowered_candidate.size())
      return false;
    if (lowered_word.compare(0, lowered_candidate.size(), lowered_candidate) !=
        0) {
      return false;
    }
    ...
    return std::all_of(suffix.begin(), suffix.end(),
                       [&](char ch) { return ch == suffix.front(); });
  }
  ...
  static bool contains_candidate_as_subsequence(const std::string &word,
    ...
    std::string lowered_word = lower_ascii(word);
    std::string lowered_candidate = lower_ascii(candidate);
    if (lowered_word.empty() || lowered_candidate.empty())
      return false;
    if (lowered_word.front() != lowered_candidate.front())
      return false;
    if (lowered_word.back() != lowered_candidate.back())
      return false;
  ...
  static size_t noisy_callable_score(const std::string &word,
    ...
    if (normal_score < impossible_score())
      return normal_score;
  ...
  static std::string
  best_callable_word(const std::string &word,
                     const std::vector<std::string> &candidates) {
    std::string best;
    size_t best_score = impossible_score();
    bool ambiguous = false;
  ...
  static size_t strong_statement_score(const std::string &word,
    ...
    if (candidate == "package" && has_keyword_prefix_damage(word, candidate)) {
      ...
      if (package_score < impossible_score())
  ...
  static std::string
  best_strong_statement_word(const std::string &word,
                             const std::vector<std::string> &candidates) {
    std::string best;
    size_t best_score = impossible_score();
    bool ambiguous = false;
  ...
  static void add_word_typo_edits(std::vector<TextEdit> &edits,
    ...
    if (index > 0 && tokens[index - 1].type == T_PLUS) {
      std::string type_replacement =
          best_word(token.value, type_words(), false);
      add_replace_if_needed(edits, token, type_replacement);
      if (!type_replacement.empty())
        return;
    }
    ...
    if (!strong_statement_replacement.empty() &&
        should_correct_statement_word(tokens, index,
                                      strong_statement_replacement)) {
      add_replace_if_needed(edits, token, strong_statement_replacement);
      return;
    }

    std::string statement_replacement =
        best_word(token.value, statement_words(), false);
    ...
    if (index + 1 < tokens.size() && tokens[index + 1].type == T_LPAREN &&
        !contains_word(callables, token.value)) {
      std::string callable_replacement =
          best_callable_word(token.value, callables);
    ...
    if (index + 1 < tokens.size() && tokens[index + 1].type == T_LPAREN) {
      ...
      std::string builtin_replacement =
          best_word(token.value, builtin_words, false);
      add_replace_if_needed(edits, token, builtin_replacement);
    }
  }

  static std::vector<TextEdit>
  plan_typo_edits(const std::vector<Token> &tokens) {
    std::vector<TextEdit> edits;
